...
try {
    ...
    //public/images/userとpublic/images/posts内の画像ファイルを削除
    ...
    //登録時にuser_idが必要なので､userデータを取得
    $sql = 'SELECT id FROM users';
    $stmt = $dbh->prepare($sql);
    $stmt->execute();
    //PDO::COLUMNを使用することで､連想配列ではなく､配列でidを取得する
    ...
    for ($i = 1; $i <= 40; $i++) {
        ...
        //登録する画像ファイル名の先頭に年月時分秒を付加
        //年月時分秒だけだと1秒間にすべての処理が終わった時にファイル名が被る可能性があるので､$i(インデックス)も付加
    ...
    echo '----commentsテーブルSET UP完了 ----' . PHP_EOL;
